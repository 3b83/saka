...
#to be changed, only allocated
direk2 = LocationGlobalRelative(-35.3629037, 149.1649067,5)
direk1 = LocationGlobalRelative(-35.3636218, 149.1649064,5)
...
mavi_havuz=LocationGlobalRelative(-35.3630831, 149.1650181, 5)#(20,20)
kirmizi_havuz=LocationGlobalRelative(-35.3627983, 149.1647635, 5)
test_location=LocationGlobalRelative(-35.361354, 149.165218, 5)
...
goto(kirmizi_baslangic.lat, kirmizi_baslangic.lon, 5)
...
out = cv2.VideoWriter("video_orig"+datetime.now().strftime("%m-%d-%Y_%H-%M-%S")+".avi",cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),25,(width,height))
out_final = cv2.VideoWriter("video_filtered"+datetime.now().strftime("%m-%d-%Y_%H-%M-%S")+".avi",cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),25,(width,height))
...
while(not red_found):
    ...
    mask1=cv2.inRange(hsv,(0,100,100),(5,255,255))
